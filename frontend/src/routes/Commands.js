...
export const all_templates_grouped = [
  {
    name: "Geometry & Transform", commands: [
      { id: nanoid(), format: "-filter $3 -resize $1x$2", template: "Resize to $1n×$2n with filter $3s{Box:Box;Lanczos:Lanczos;Cubic:Cubic;Catrom:Catrom;Triangle:Triangle;} (@{v[0] * v[1]}@ pixels)", value: ["1000", "1000", "Lanczos"], name: "Resize", type: "in", tooltip: "Resize the image to the specified width and height in pixels." },
      { id: nanoid(), format: "-filter $2 -resize @{w*$1}@x@{h*$1}@", template: "Scale $1n× with filter $2s{Box:Box;Lanczos:Lanczos;Cubic:Cubic;Catrom:Catrom;Triangle:Triangle;}", value: ["2", "Lanczos"], name: "Scale", type: "in", tooltip: "Scale the image to the specified facto." },
      { id: nanoid(), format: "-filter $3 -resize @{w*($1/$2)}@x@{h*($1/$2)}@", template: "Resize $2s{width:w;height:h;} to $1n with filter $3s{Box:Box;Lanczos:Lanczos;Cubic:Cubic;Catrom:Catrom;Triangle:Triangle;}", value: ["1000", "w", "Lanczos"], name: "Resize Side", type: "in", tooltip: "Resizes a given side of the image to the specified value while preserving the aspect ratio." },
      { id: nanoid(), format: "-filter $3 -resize $1x$2!", template: "Stretch to $1n×$2n with filter $3s{Box:Box;Lanczos:Lanczos;Cubic:Cubic;Catrom:Catrom;Triangle:Triangle;}", value: ["1000", "1000", "Lanczos"], name: "Stretch", type: "in", tooltip: "Resizes to fit specified dimensions by stretching the image." },
      { id: nanoid(), format: "-filter $3 -resize @{w * $1}@x@{h * $2}@", template: "Scale width ×$1n and height ×$2n with filter $3s{Box:Box;Lanczos:Lanczos;Cubic:Cubic;Catrom:Catrom;Triangle:Triangle;}", value: ["2", "1.5", "Lanczos"], name: "Scale Sides", type: "in", tooltip: "Scale image width and height independently." },
